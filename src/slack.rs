    ...
    while let Some(token) = tokens.pop_front() {
        ...
        if current_chars + token_chars + closing.chars().count() > MAX_TEXT_CHARS {
            if !current_has_content && rendered.starts_with('<') && rendered.ends_with('>') {
                let mut replacement = tokenize(&readable_link(&rendered));
...
mod tests {
    ...
    #[test]
    fn oversized_link_fallback_keeps_escape_entities_atomic() {
        let prefix = "label (https://example.com/";
        let padding = "x".repeat(MAX_TEXT_CHARS - prefix.chars().count() - 1);
        let link = format!("<https://example.com/{padding}&amp;x|label>");
        let expected = format!("{prefix}{padding}&amp;x)");
        ...
        assert!(!chunks.iter().any(|chunk| chunk.starts_with("amp;")));
    }
