...
pub(super) const DRAFT_SETUP_FAILURE: &str =
    "Relay could not prepare its draft workspace. Check the local logs, then resend.";
...
pub(super) async fn handle(ctx: &Ctx, mut job: Job) {
    ...
    // Relay exposes the draft boundary. The selected agent's own configuration
    ...
    if let Some(draft_write_dir) = draft_write_dir {
        instructions.push_str(&format!(
            "\n\nTo propose a recurring job, write one complete validated Markdown runbook to {}/<lowercase-slug>.md. This drafts directory is the only Relay-owned path you may write. Your agent configuration decides whether the user-owned context directory is editable. A draft remains inactive until the allowlisted user approves its exact revision. Never write to the installed jobs directory, Relay configuration, or Relay state.",
            draft_write_dir
        ));
    }
...
async fn prepare_voice(ctx: &Ctx, job: &Job) -> std::result::Result<String, VoicePreparationError> {
    ...
    let Some(voice) = &ctx.voice else {
        return Err(VoicePreparationError::User {
            ...
            reply: "Voice messages are unavailable. Set OPENAI_API_KEY and restart Relay, or send text instead.",
