...
pub struct Config {
    ...
    /// Authoritative resolved locations for all Relay-owned runtime data.
    #[serde(skip)]
...
impl Config {
    ...
    pub fn load(path: &str) -> Result<Config> {
        Self::load_with_paths(path, RelayPaths::discover()?)
    }

    pub(crate) fn load_with_paths(path: &str, default_paths: RelayPaths) -> Result<Config> {
    ...
    pub fn validate_job_workdir(&self, workdir: &Path) -> Result<()> {
        ...
        let protected_paths = [
            ("Relay home", self.paths.root.as_path()),
...
fn validate_runtime_outside_assistant(cfg: &Config) -> Result<()> {
    ...
    let relay_home = resolved_absolute("RELAY_HOME", &cfg.paths.root)?;
    if paths_overlap(&assistant, &relay_home) {
        bail!(
...
mod tests {
    ...
    fn config() -> Config {
        ...
        Config {
            ...
            paths: RelayPaths::from_root(root.join("runtime")).unwrap(),
