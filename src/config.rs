...
impl Config {
    ...
    // allow writes, so every job workdir must stay clear of Relay-owned paths,
...
fn reject_uncontained_route_profile(profile: &PermissionProfile) -> Result<()> {
    if profile.capability == PermissionCapability::FullAccess {
        bail!(
            "route permission profile {:?} uses full-access, which cannot prevent direct writes to Relay jobs or state",
            profile.name
        );
    }
    Ok(())
...
pub enum PermissionCapability {
    ReadOnly,
    Workspace,
    /// Defer to the backend's own permission configuration. Relay passes no
