...
fn load_config(path: &str) -> Result<config::Config, CliError> {
    ...
    let cfg = config::Config::load(path).map_err(|error| {
        CliError::configuration(
            format!("configuration at {path} could not be loaded; run `relay doctor` for details"),
            error,
        )
    })?;
...
fn paths_value(cfg: &config::Config) -> Value {
    json!({
        "relay_home": cfg.paths.root.to_string_lossy(),
...
pub(crate) fn format_paths(cfg: &config::Config) -> String {
    ...
    let mut output = String::from("relay paths\n");
    for key in [
        "relay_home",
        "config",
        "default_config",
