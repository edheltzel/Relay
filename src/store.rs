...
impl Store {
    pub fn open(paths: &RelayPaths) -> Result<Self> {
    ...
    fn migrate_legacy_state(&mut self, _fail_before_commit: bool) -> Result<()> {
        ...
        let source_sha256 = crate::util::hex_lower(Sha256::digest(&data));
...
mod tests {
    ...
    fn temp_paths() -> (String, String) {
        let base = std::env::temp_dir().join(format!("relay-store-test-{}", Uuid::new_v4()));
        (
  ...
  "telegram:dm:7":{"uuid":"legacy","started":true,"backend":"codex"}
    ...
    fn database_open_failure_is_actionable_before_state_is_available() {
        ...
        let blocked_database = blocker.join("relay.db");
