...
Relay is a local Rust gateway that turns Claude Code, Codex, or Pi into a
personal assistant available through iMessage, Telegram, and Slack. It also
runs validated Markdown jobs on a schedule. Relay owns messaging, routing,
...
```text
message channel -> Relay gateway -> agent CLI -> Relay gateway -> message channel
```

Relay is one binary and one local process. It opens no inbound server port.
...
Relay does not implement an agent loop, tool runner, plugin system, or model
...
Relay is a single crate. Module boundaries, not separate packages, keep
...
Relay divides ownership among three systems:

| Owner | Responsibilities |
| --- | --- |
| Relay runtime | channels, allowlists, routing, scheduling, canonical history, cursors, session mappings, audit, retries, and delivery |
| Assistant repository | user-owned `SOUL.md`, shared instructions, context, evals, jobs, optional project skills, and the Relay-managed capability skill |
...
`instructions` always contains the same ordered system sections: Relay-owned
base policy, user-owned system identity, and Relay-resolved workspace paths.
...
error text. Commands that mutate runtime state reject JSON mode because Relay
cannot always know whether an interrupted external mutation is safe to retry.
Resolved-path output reads every Relay-owned runtime location from the loaded
`RelayPaths` owner. It adds the selected config and user-owned assistant paths
without reconstructing runtime defaults. Although job runs now share `relay.db`
...
Relay uses separate stores because they have different update and query needs:

| Store | Purpose |
| --- | --- |
| `$RELAY_HOME/state.json` | retained legacy cursor and session source for one-time migration |
| `$RELAY_HOME/relay.db` | transactional conversation, delivery, approval compatibility, job-run history, channel cursors, and channel-qualified backend sessions |
| `$RELAY_HOME/state.json.slack-inbox.db` | Slack Socket Mode inbox committed before envelope acknowledgement |
| `$RELAY_HOME/audit.jsonl` | append-only operational audit events |
| `$RELAY_HOME/run/` | advisory job locks |
| `$RELAY_HOME/cache/` | disposable agent handoff files |
| assistant Git repository | user-owned identity, context, evals, jobs, and project resources |

`state_path` remains a compatibility input for one-time migration. Relay never
...
`RELAY_HOME` selects the root and defaults to `~/.relay`. Config, startup, jobs,
history, audit, and channels consume the resolved `RelayPaths` value instead of
...
when present. `--config` selects a file without changing `RELAY_HOME`;
otherwise the config is `$RELAY_HOME/config.toml`.
...
- removed Relay-owned agent permission settings;
...
`relay doctor` adds environment checks for agent binaries, database access,
...
`History` connection to `relay.db`, one runner per required backend, and one
...
On the first run for a channel, Relay asks the adapter for its latest cursor and
...
```text
 ...
 5. HISTORY INSERT       persist accepted inbound message in relay.db
...
Accepted inbound messages are inserted in `relay.db` before local commands or
...
Different threads and channels may run concurrently. If a queue is full, Relay
...
| --- | --- | --- |
| Relay-owned base policy | Relay; highest among composed sections | native system or developer instructions |
| User-owned system identity | `SOUL.md`; below Relay policy | native system or developer instructions |
| Resolved workspace paths | Relay-owned path data | native system or developer instructions |
| Fresh message context | untrusted current-turn data | ordinary prompt input |

The base policy is limited to Relay delivery, instruction-boundary, context,
...
Relay does not inject every context file. The selected backend decides what to
...
If a backend reports that a resumed session is missing, Relay rotates the stored
...
parallel. `AckState` tracks in-flight and completed row IDs. Relay advances the
...
Relay chooses Claude's initial session ID and marks it started before execution
...
Codex owns its thread ID. Relay reads `thread.started.thread_id` from JSONL and
...
`relay init` installs one canonical capability at `skills/relay/`. Relative
directory links expose it at `.agents/skills/relay` for Codex and Pi and at
`.claude/skills/relay` for Claude Code. This matches each runtime's project
...
starts ignore that source even though Relay deliberately leaves it in place as
...
5. ENQUEUE       record one due occurrence in relay.db
...
Relay does not catch up occurrences missed while offline. A clock jump queues at
...
Relay starts a fresh restricted session with the original runbook, candidate
...
Relay does not override sandbox, approval, or tool configuration for chats.
...
Every enabled job should be treated as code execution by the Relay service user.

### Network Rule

Relay accepts no inbound network connection:
...
Relay targets stable Rust on macOS and Linux. iMessage is macOS-only.
