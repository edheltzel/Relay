...
Relay is an always-on AI assistant that does real work for you. It runs 24/7 on
...
cron trigger        -> Relay gateway -> Claude Code or Codex -> result to your chat
```
...
`relay doctor` checks shared paths and backend binaries, then only the selected
...
bounded question in `relay.db` before sending the same plain numbered list on
...
[GitHub Releases](https://github.com/edheltzel/relay/releases). Release notes
...
Every route selects a named Relay permission profile. The default is the built-in
...
and deny rules), and Codex uses its configured default sandbox. Relay passes no
...
therefore write, every job work directory must stay clear of Relay-owned paths.
...
```bash
relay job validate --config config.toml
relay job list --config config.toml
relay job show repo-review --config config.toml
relay job run repo-review --config config.toml
relay job runs repo-review --config config.toml
```
...
that overlap Relay-owned paths. Invalid jobs are disabled individually and do
not stop messaging or other valid jobs.

Manual runs execute in the invoking CLI process. Relay records and claims the
...
remain visible through `relay job runs`; manual results are printed to the
...
`relay job runs`. Delivery retries up to three times with backoff from the stored
...
runbook to the identity-specific inbox Relay provides beneath `drafts_dir`, which
defaults to `~/.relay/drafts`. Relay gives the backend only that opaque inbox as
...
outbound reply, Relay reconciles every unrecorded revision in that route's inbox.
It validates each filename,
complete contents, work directory, timeout, backend, triggers, and named
permission profile. Invalid files, symlinks, path escapes, protected Relay paths,
...
SHA-256 revision shown in the question. Relay stores the proposal bytes and both
identities in `relay.db`. It revalidates the current draft and configured ceiling
