...
```
iMessage (chat.db)  ->  relay (poll loop)  ->  claude -p  ->  iMessage reply
```
...
## Why relay: it's actually Claude Code

relay is not an agent. It is a thin pipe to the real `claude` binary. Your texted
...
feature, relay has it the same day, for free, with no code change.
...
maintains. relay has no agent loop of its own to inherit. It hands your message
to `claude` and sends back what `claude` says.

A concrete payoff: relay bills your **Claude subscription**, because it just runs
...
relay is a hedge against that. The durable layer is the one you own: how messages
reach you, how your memory is stored, how work is routed. relay keeps that layer
...
(including why relay's file-based memory beats an opaque memory database).

## Requirements

- macOS with iMessage signed in.
- **Full Disk Access** for your terminal (System Settings -> Privacy &
  Security -> Full Disk Access) so relay can read `chat.db`.
...
relay runs Claude Code with `--permission-mode bypassPermissions` so it can act
