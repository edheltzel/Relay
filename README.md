...
optional project skills. Relay owns channels, schedule evaluation, history,
...
Install the latest prebuilt release to `~/.local/bin/relay`:
...
and `jobs/`, then records its path in `~/.relay/config.toml`. New configs use
...
Relay leaves sandbox, approval, and tool permissions to the selected backend.
...
and [running as a service](docs/services.md) to keep Relay online with `launchd`
or `systemd`.
...
permissions you give your backend. Relay does not override agent permissions, so
...
Relay is a Rust project. After cloning the repository, run the same core checks
used by CI:
