...
impl Fixture {
    fn new(name: &str) -> Self {
        let root = std::env::temp_dir().join(format!("relay-json-{name}-{}", Uuid::new_v4()));
...
#[test]
fn paths_use_relaypaths_for_the_selected_runtime_home() {
    let fixture = Fixture::new("relay-paths");
    let relay_home = fixture.root.join("isolated-runtime");
    let output = fixture
        .command()
        .arg("paths")
    ...
    assert_eq!(data["relay_home"], relay_home.to_string_lossy().as_ref());
    assert_eq!(
        data["default_config"],
        relay_home.join("config.toml").to_string_lossy().as_ref()
    );
    assert_eq!(
        data["config"],
        fixture
    ...
    assert_eq!(
        data["database"],
        relay_home.join("relay.db").to_string_lossy().as_ref()
    );
    assert_eq!(
        data["state"],
        relay_home.join("state.json").to_string_lossy().as_ref()
    );
    assert_eq!(
        data["audit_log"],
        relay_home.join("audit.jsonl").to_string_lossy().as_ref()
    );
    assert_eq!(
        data["jobs_run"],
        relay_home.join("run").to_string_lossy().as_ref()
    );
    assert_eq!(
        data["slack_inbox"],
        relay_home
    ...
    assert_eq!(
        data["cache"],
        relay_home.join("cache").to_string_lossy().as_ref()
    );
}
...
fn job_runs_json_omits_stored_content_fields() {
    ...
    json_stdout(&initial);
    let database = fixture.home.join(".relay/relay.db");
