...
relay is that gateway, and it is a long-term hedge against single-provider
lock-in.
...
So relay splits the world into two parts:
...
- **The agent is a swappable slot.** Today relay drives Claude Code, because it
  is the best first-party agent available and relay uses it natively rather than
  wrapping it. Tomorrow the same gateway can route to a different agent when a
  different agent is better for the task.

## Why this is not a contradiction

relay's other headline is "it's actually Claude Code, not a third-party wrapper."
...
world. relay keeps the world yours and lets the agent be a choice you remake
whenever you want.
...
single provider owns your workflow." relay is the worked example: small, legible,
