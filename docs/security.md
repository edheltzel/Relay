...
| `inherit` | No Relay mode or tool filters | No Relay sandbox override | No Relay tool allowlist | Defer to the operator's backend configuration |
...
These profiles control the local filesystem and process capabilities that Relay
...
Relay rejects raw backend permission flags in TOML so a route cannot
quietly bypass the shared local policy model.

## Job permissions

Jobs always inherit the backend's own permission configuration. They do not
select a Relay permission profile. Relay compensates by requiring a fixed,
existing work directory and rejecting overlap with Relay-owned files, including
