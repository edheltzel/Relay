...
user-owned Markdown runbook in `~/.relay/jobs/`. TOML frontmatter defines
execution policy; the Markdown body is sent verbatim to a fresh backend
session.

## Create a job

Create `~/.relay/jobs/repo-review.md`:
...
Unknown fields are errors. A job work directory may not overlap Relay's config,
