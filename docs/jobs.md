...
root, loaded Relay config, state, database, audit log, drafts, or job lock paths.
...
```sh
mkdir -p ~/.relay/workspaces/daily-inbox-triage
```
...
runbook to its origin-specific drafts inbox. Relay creates the opaque inbox and
...
!!! warning
    ...
    every enabled job as code execution by the Relay service user, keep job work
