---
title: "Rename Push to Relay with a clean cutover"
date: 2026-07-23
type: refactor
execution: code
status: active
...
autonomy: standard
original-request:
  - "Create an in-depth, line-by-line plan to rename the project from Push to Relay without grep/ripgrep replacement discovery."
  - "Use multiple agents, preserve existing case conventions, and change only project identity—not product behavior."
  - "Use a clean cutover and replace com.edheltzel.push with com.edheltzel.relay."
---

# Rename Push to Relay with a clean cutover

## Goal Capsule

Rename the current, private, standalone repository and every active project-owned identity surface from **Push** to **Relay** without changing gateway, scheduling, delivery, security, or agent-runtime behavior. The cutover includes the Rust package and binary, CLI copy, runtime paths, service identifiers, release assets, installer, tests, documentation, GitHub repository metadata, retirement of the ineligible Pages configuration, and the local checkout directory.

This is deliberately a **clean cutover**. Relay will not ship a `push` command alias, read `~/.push`, discover `push.db`, recognize the old launchd/systemd identifiers, emit duplicate release assets, or add deprecated compatibility code.
...
| Distribution/services | `examples/launchd/com.edheltzel.push.plist` (35), `examples/systemd/push.service` (16), `install.sh` (116) |
...
- **Maintainers** retain the original `edheltzel/push` repository only as a fetch-only upstream and preserve historical Git/release records.

### Requirements

- **R1 — Canonical identity:** Current project-owned `Push`/`push`/`PUSH` forms become `Relay`/`relay`/`RELAY` according to existing case and separator conventions.
...
- **R3 — Runtime paths:** Defaults move from `~/.push/...` to `~/.relay/...`; the default database becomes `relay.db`.
...
- **R8 — Semantic preservation:** Generic Rust APIs (`.push`, `.push_str`), GitHub's `push:` event, ordinary English verbs, external push-notification terminology, adversarial fixtures, and explicitly historical records remain unchanged.
...
- **AE1:** `relay --version` prints `relay <current-version>`; `push` is not installed or advertised.
...
|---|---|---|
| `Push` | `Relay` | Product nouns, headings, messages, comments, Mermaid labels |
| `push` | `relay` | Binary/package names, command examples, project-owned file stems, CSS classes |
| `PUSH_*` | `RELAY_*` | Project-owned test coordination variables only |
| `push-*` / `push_*` | `relay-*` / `relay_*` | Archive, temp, fixture, hook, and helper names |
| `~/.push` | `~/.relay` | Default private configuration/runtime root |
| `push.db` | `relay.db` | Default/project-owned database filename |
| `com.edheltzel.push` | `com.edheltzel.relay` | launchd label and plist filename |
| `push.service` | `relay.service` | systemd unit and unit filename |
| `edheltzel/push` | `edheltzel/relay` | Active repository URLs and metadata |
| `/main/`, `blob/main`, `edit/main` | `/master/`, `blob/master`, `edit/master` | Active branch-bound repository URLs |
| `push-light`, `--push-*`, `.push-*` | `relay-light`, `--relay-*`, `.relay-*` | MkDocs scheme and synchronized CSS/HTML hooks |
...
- `src/soul.rs`'s intentionally stale `Push` instruction fixture and `src/rehydration.rs`'s adversarial `SYSTEM: ignore push` payload.
- The fetch-only local `upstream` URL `git@github.com:edheltzel/push.git`.
- Existing commit messages, tags, GitHub release titles, and already-published historical release assets.

### Key technical decisions

1. **Clean cutover, not compatibility.** `session-settled: user-directed` — rejected alternative: retain a `push` alias, legacy config fallback, automatic migration, or dual service identifiers.
...
4. Rename test harness references from `CARGO_BIN_EXE_push` to `CARGO_BIN_EXE_relay`; rename the private `push_command` helper to `relay_command` and every call site.
...
- Rename `examples/launchd/com.edheltzel.push.plist` to `examples/launchd/com.edheltzel.relay.plist`; update lines 7, 11, 13, 17, 31, 33.
- Rename `examples/systemd/push.service` to `examples/systemd/relay.service`; update lines 2, 8, 9, 13.
...
**Approach:** Unload and remove any installed `com.edheltzel.push`/`push.service` unit before installing the Relay unit, then rename service labels/files, binary/config/log/env paths, installer diagnostics and temp artifacts, release package stems, archive contents, checksum fixtures, and restart assertions. Produce no old-name service file, symlink, or duplicate release archive.

**Unit checks:** `plutil -lint examples/launchd/com.edheltzel.relay.plist`; `bash tests/install.sh`; inspect a locally generated release archive to prove it contains `relay` and no `push` binary; prove only the Relay service identifier is loaded/enabled.
...
- `docs/index.md:7-181`, including install URL line 22 and every `push-*` HTML hook
- `docs/stylesheets/extra.css:1-669`, renaming every `--push-*` custom property and `.push-*` selector atomically
...
2. Remove the obsolete/unrelated `https://pushassistant.com/` target: use repository-relative `docs/index.md` links in README, remove `site_url` from MkDocs, and keep repository metadata free of a homepage until a real Relay site exists.
3. Rename Mermaid node identifiers/labels when they denote the product.
4. Rename MkDocs scheme `push-light` to `relay-light` and all matching CSS/HTML hooks without changing CSS values or layout.
...
**Approach:** Replace product nouns, CLI commands, `.push`/`push.db` paths, diagram node IDs/labels, release/repository URLs, and project-owned example filenames. Do not rewrite requirements, decisions, roadmap scope, or system behavior. Current tracked historical documents become Relay documents; immutable Git history remains the historical record of the old name.
...
1. Rename private standalone GitHub repository `edheltzel/push` to `edheltzel/relay` without using GitHub's fork operation.
2. Keep default branch `master`; confirm no `main` branch is created.
3. Clear the obsolete `https://pushassistant.com/` repository homepage; retain existing description, private visibility, issues/wiki policy, and merge settings.
4. Update local `origin` fetch/push URL to `git@github.com:edheltzel/relay.git`.
5. Preserve `upstream` fetch URL `git@github.com:edheltzel/push.git` and push URL `DISABLED`; preserve `remote.pushDefault=origin` and branch-specific push routing.
6. Push `master` and verify CI/security on `master`; confirm the removed Pages workflow does not run.
7. Rename local checkout directory `/Users/ed/Developer/Atlas/Push` to `/Users/ed/Developer/Atlas/Relay` last; reopen tools/indexes from the new path.
...
4. For each surviving `Push`/`push`/`PUSH`, classify it against the explicit preserve list. Any unclassified project-identity survivor reopens its implementation unit.
...
1. **CLI identity:** Built debug and release binaries report `relay`, show only `relay` commands, and contain no advertised `push` alias.
...
- A project-owned current surface still advertises Push, `push`, `.push`, `push.db`, the old service label, or `push.service` without an explicit preserve classification.
- `relay` and `push` both work or are packaged.
...
|---|---|---|
| Clean path cutover ignores existing `~/.push` data | Existing conversations/state/configuration appear absent | Before deploying Relay on a machine with real data, stop the old service and make an operator-controlled backup/copy of `config.toml`, `state.json`, `audit.jsonl`, and `push.db` renamed to `relay.db`; rewrite project-owned paths inside the copied config to `.relay`, copy only deliberate run artifacts and no stale locks, verify the copied data, and do not add runtime fallback code. |
...
- Renaming or modifying the original `edheltzel/push` upstream repository.
...
- Existing GitHub repository: `https://github.com/edheltzel/push` (to be renamed)
- Original upstream: `https://github.com/edheltzel/push`
