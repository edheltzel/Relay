...
Relay supports Telegram private chats through Bot API long polling. It makes
...
If the bot previously used a webhook, remove it before starting Relay because
Telegram does not allow `getUpdates` while a webhook is active. The first Relay
start discards pending updates, so send a new message after the gateway reports
that it is running.

Export the token in the environment that starts Relay:
...
config file. Relay never prints the token. Run:

```sh
relay doctor --config /absolute/path/to/config.toml
relay --config /absolute/path/to/config.toml
```
