...
Fill both in before running Relay. Relay derives `SOUL.md`, `context/`, and `jobs/` from
...
or `pi`. Relay does not set sandbox, approval, permission-mode, or tool flags.
...
Relay runs `pi --print --mode json`, stores the session ID from Pi's JSON event
...
private. `relay init` creates new config files with mode `0600` on Unix. The
...
Permissions belong to the agent, not the gateway. Relay invokes Claude Code,
...
| `reply_marker` | `"\n\n-- sent by relay"` | iMessage loop-prevention marker |
...
| Setting | Default | Purpose |
| --- | --- | --- |
| `drafts_dir` | `~/.relay/drafts` | Inactive agent-authored proposals |
...
Relay validates that state, sessions, the assistant root, drafts, locks, the
