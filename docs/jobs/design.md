...
`~/.relay/drafts/`, which Relay adds as a Relay-owned writable root for contained
...
writes to Relay-owned files. Job work directories may not overlap the assistant
...
outbound reply, Relay reconciles unrecorded revisions in that origin's inbox.
