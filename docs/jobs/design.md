...
- Relay has one long-running gateway process with no inbound server port.
  ...
  store.
- Claude Code, Codex, and Pi own their permission controls. Relay passes no
...
claims, and the gateway owns scheduled runs. Relay installs draft jobs only
...
`~/.relay/drafts/`. Relay creates the inbox and includes its path in the
