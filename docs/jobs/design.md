...
**Author:** Ed Heltzel  **Date:** 2026-07-12

## Summary

Relay jobs are user-owned Markdown runbooks stored under `~/.relay/jobs/`. A job
...
and ledger ship, Relay records each run in SQLite before execution, allowing
scheduling and delivery to remain durable without turning Relay into an agent
runtime.
...
- Relay remains one local process with no inbound server port.
- Claude Code and Codex have different permission controls, but a job selects
  only a named Relay permission profile.
...
Each job is one UTF-8 file at `~/.relay/jobs/<job-name>.md`. The file stem is the
...
- `permission_profile`: required named profile from Relay configuration.
...
read-only `relay job validate` command. If any installed job is invalid, startup
...
or a new profile. Relay resolves the profile to backend controls only after the
...
Each run starts a fresh Claude or Codex session. Relay supplies the composed
...
the global worker limit. `relay job run` is a short-lived local producer: it
...
Relay is the only writer to the run ledger: the gateway owns execution state and
...
`~/.relay/drafts/`, but activation must revalidate and atomically install the
exact approved revision into `~/.relay/jobs/`.
...
2. Add the SQLite run ledger, make the gateway the sole long-running executor,
   ...
   `relay log`. From this step onward every run is recorded before execution.
