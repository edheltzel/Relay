...
`relay.db`, `<assistant_root>/jobs`, and the local per-job lock directory. Invalid job files
...
resolves. Keep `relay.db`, `<assistant_root>/jobs`, and `jobs_run_dir` on persistent local
...
write tools. Relay rejects `full-access` for unattended routes and jobs because
