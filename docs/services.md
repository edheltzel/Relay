...
`relay.db`, `jobs_dir`, and the local per-job lock directory. Invalid job files
...
resolves. Keep `relay.db`, `jobs_dir`, and `jobs_run_dir` on persistent local
...
Use `relay job runs` to distinguish execution state from delivery attempts.
...
write tools. Relay rejects `full-access` for unattended routes because it cannot
enforce the drafted-job boundary. Jobs inherit backend permissions and are
kept away from Relay-owned paths by work-directory validation.
