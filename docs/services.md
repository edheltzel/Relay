...
- write access to `assistant_root/jobs/` for Relay to install approved drafts;
...
`chmod 600 ~/Library/LaunchAgents/com.edheltzel.relay.plist`. Omit the key when
...
Keep `~/.relay/config.toml` at mode `0600` because it contains the Telegram bot
...
with owner-only permissions. Relay exposes only the route's origin-specific
...
for unattended use. Relay passes no sandbox, approval, or tool overrides. Jobs
are kept away from Relay-owned paths by work-directory validation.
