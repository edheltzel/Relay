...
Relay supports one-to-one Slack app direct messages. It receives events through
...
You can instead set `slack.app_token` and `slack.bot_token` in the private Relay
...
At runtime Relay checks the bot token with Slack's
...
Relay validates the workspace, message shape, direct-message channel type,
...
at the originating message. Relay uses
...
Before acknowledging an accepted Socket Mode envelope, Relay commits it to a
...
- If `Slack app token` fails in `relay doctor`, set `SLACK_APP_TOKEN` or
  `slack.app_token`.
...
- Reconnect messages are expected because Slack refreshes Socket Mode
  connections periodically. Relay's dedicated receiver reconnects automatically.
