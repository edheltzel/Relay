...
This doc explains how relay is put together and, more importantly, *why* we made
...
relay only ever *reads* (`chat.db`) and *runs local commands* (`claude`,
...
This is deliberate, because relay is a security-focused gateway that handles an
...
only way to give relay work is to send a message through a channel it already
trusts (the allowlist), so the trust boundary is the messaging account, not a
network endpoint.

Consequence for deployment: relay runs anywhere it can read a message source and
...
relay is a single Rust process on your Mac. It reads incoming iMessages from the
...
read it, or correct a single wrong fact by hand. relay takes the opposite bet.
...
relay stays tiny and inherits all of that. The cost is one process per message;
for a personal assistant that is fine.

## No third-party agent layer

This is relay's main positioning. relay has no agent loop of its own. It is a pipe
...
whatever Anthropic ships in Claude Code itself. relay inherits nothing because it
wraps nothing. It runs `claude` and relays the answer.

Practical consequences:

- **Billing**: relay runs `claude` with your environment, so it uses your Claude
...
- **Features for free**: skills, MCP, permission modes, `CLAUDE.md` — anything
  Claude Code gains, relay gains the same day with no code change.
...
relay runs Claude Code with `--permission-mode bypassPermissions` because a
