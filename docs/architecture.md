...
Relay is one local Rust process. It polls one or more configured iMessage and
...
Relay polls channel state and shells out to local agent commands. It opens no
...
Pi tool allowlists implement Relay permission profiles, but Pi has no native
...
rejected because they cannot enforce that boundary. Relay snapshots
...
use the job draft approval flow. Relay does not write the footer to the
...
Routes select a named Relay permission profile. `restricted` is the default.
Relay rejects `full-access` for routes because backend bypass modes cannot
protect Relay-owned files. Claude receives a fixed tool allowlist and denies
...
configuration and must use a work directory that does not overlap Relay-owned
