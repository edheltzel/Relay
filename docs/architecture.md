...
permission profiles; full-access routes and jobs are rejected. Relay snapshots
...
Routes select a named Relay permission profile. `restricted` is the default and
jobs may use only explicitly allow-listed profile names. Relay rejects
`full-access` for routes and jobs because backend bypass modes cannot protect
Relay-owned files. Claude receives a fixed tool allowlist and denies Bash;
