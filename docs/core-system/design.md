...
- Build a new filesystem sandbox. Relay uses the existing named permission
...
Propose job changes through Relay's approval workflow.
```

Claude receives the composed text as appended system instructions. Codex
receives it as developer instructions. Relay never writes resolved machine paths
