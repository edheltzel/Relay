...
`relay` is a small Rust binary that lets you text a personal assistant. It polls
...
system, cron, personas, parallel subagents. relay is deliberately narrower and
...
relay's bet: memory that is human-legible, hand-editable, and version-controlled
...
Headless Claude Code has no human to approve tool calls, so relay runs with
